...
@section('page_subtitle', 'Buat dan atur mitra Anda di sini.')
            ...
            <tbody class="divide-y border-t">
                @forelse($partners as $index => $partner)
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="px-8 py-6">
                            <img src="{{ $partner->logo_url }}" class="w-16 h-16 rounded-xl object-cover shadow-sm" alt="Logo {{ $partner->name }}">
                        </td>
                        <td class="px-8 py-6">
                        ...
                        <td class="px-8 py-6">
                            <div class="flex justify-center items-center gap-2">
                                ...
                                <form action="{{ route('admin.partners.destroy', $partner->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2.5 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition shadow-sm" onclick="return confirm('Yakin ingin menghapus?')">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
